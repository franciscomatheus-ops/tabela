...
function Res() {
    ...
    let qativ = 0;
    for (let x = 1; x <= tbls; x++) {
        let res = document.getElementById(`res${x}`).textContent.trim() != '' ? parseInt(document.getElementById(`res${x}`).textContent) : 0;
        document.getElementById(`res${x}`).textContent.trim() != '' ? qativ++ : qativ += 0;
        soma += res;
    }
    document.getElementById('res').textContent = soma > 0 ? soma : '';
    console.log((soma  / qativ).toFixed(1).replace('.', ','));
